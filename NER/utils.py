...
def news_split(test_size=0.3):
    ...
    for entry in train:
        ...
        train_str += entry[0] + " " +entry[1]
        train_str += "\n"
    
    test_str = ""
    for entry in test:
        if not entry[0]:continue
        test_str += entry[0] + " " +entry[1]
        test_str += "\n"

    text_file = open(data_path + "news_train.conll", "w",encoding="utf-8")
    text_file.write(train_str)
    text_file.close()

    text_file = open(data_path + "news_test.conll", "w",encoding="utf-8")
